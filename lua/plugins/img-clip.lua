return {
	{
		...
		opts = {
			...
			default = {
				...
				-- file_name = function()
				-- 	local file = vim.fn.expand("%:t:r") -- Get current file name without extension
				-- 	local date_format = "!%Y%m%d" -- Ensure proper date formatting
				-- 	return string.format("%s-%s", file, os.date(date_format))
				-- end,
				dir_path = function()
